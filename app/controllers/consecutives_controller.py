...
class ConsecutivesController:
    ...
    def register_routes(self, app: FastAPI):
        ...
        @app.get(
            "/api/users/{user_id}/organization/{organization_id}/consecutives",
        ...
        @app.get(
            "/api/users/{user_id}/organization/{organization_id}/consecutives/{consecutive_id}",
        ...
        async def get_consecutive(
            ...
            try:
                result = consecutive_service.get_consecutive(organization_id, user_id, consecutive_id)
                if not result:
        ...
        @app.get(
            "/api/users/{user_id}/organization/{organization_id}/terminals/{terminal_id}/consecutives/{document_type_id}",
            response_model=ConsecutiveResponse,
            tags=["consecutives"],
        ...
        async def create_consecutive(
            ...
            try:
                return consecutive_service.create_consecutive(organization_id, user_id, body)
            except ValueError as e:
                raise HTTPException(status_code=422, detail=str(e))
            except Exception as e:
                raise HTTPException(status_code=500, detail=str(e))
        ...
        @app.patch(
            "/api/users/{user_id}/organization/{organization_id}/consecutives/{consecutive_id}/status",
            response_model=ConsecutiveResponse,
            tags=["consecutives"],
        ...
        async def update_consecutive_status(
            ...
            try:
                result = consecutive_service.update_consecutive_status(
                    organization_id, user_id, consecutive_id, body.status
                )
                if not result:
                    raise HTTPException(status_code=404, detail="Consecutive not found")
                return result
